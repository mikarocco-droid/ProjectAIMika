...
PROMPT_MICRO_SCAN_GOAL = """Analyse football match — {n} images extraites de la fenetre {debut} a {fin} (juste avant une reprise au centre du terrain detectee), numerotees frame_0 a frame_{n_moins_1} dans l'ordre chronologique.

ETAPE 1 — OBLIGATOIRE : pour CHAQUE frame, dans l'ordre, assigne un code COURT parmi : "normal", "attente" (joueurs statiques/calmes), "ballon_filets" (ballon dans les filets/filets deformes), "celebration" (posture de joie active), "gardien_action" (gardien qui plonge/degage), "touche" (remise en touche). Un seul mot par frame, PAS de phrase. Ne saute AUCUNE frame — un signal important peut n'apparaitre que sur une seule image parmi toutes celles fournies.

ETAPE 2 — Question finale : Un BUT a-t-il ete marque dans cette fenetre ? Si oui, a quel instant approximatif (en secondes depuis le debut de la fenetre, 0 = premiere image) ?

Regles pour l'etape 2 — a lire attentivement :
...
{{"codes_par_frame": {{"0": "normal", "1": "attente", "...": "..."}}, "is_goal": true ou false, "timestamp_dans_fenetre": <secondes ou null>, "confidence": <0.0-1.0>, "evidence": "<decris precisement : position du ballon par rapport aux filets/ligne, presence/absence de celebration, et tout signal negatif observe>"}}
Le dictionnaire "codes_par_frame" DOIT contenir exactement {n} entrees (cles "0" a "{n_moins_1}"), une par frame fournie.
...
def micro_scan_confirm_goal(video_path, window_start, window_end,
    ...
    prompt = PROMPT_MICRO_SCAN_GOAL.format(
        n=len(parts), n_moins_1=len(parts) - 1,
        debut=f"{_mm_d:02d}:{_ss_d:02d}", fin=f"{_mm_f:02d}:{_ss_f:02d}",
    )
    ...
    codes_par_frame = data.get("codes_par_frame", {})

    if codes_par_frame:
        print(f"  [MICRO_SCAN] Codes par frame ({len(codes_par_frame)}/{len(parts)}) :")
        for i, off in enumerate(offsets_valides):
            _t_frame = window_start + off
            _code = codes_par_frame.get(str(i), "?")
            print(f"    frame_{i} (t≈{_t_frame:.0f}s) : {_code}")
    ...
    return {
        ...
        "evidence": evidence,
        "codes_par_frame": codes_par_frame,
        "n_appels_gemini": 1,
    }


# ─────────────────────────────────────────────────────────────────────────
# MICRO-SCAN v2 — re-vote sur la formation RESTART_KO (V5.2, 12/09/2026)
# ─────────────────────────────────────────────────────────────────────────
# Remplace l'approche "chercher le ballon dans les filets/la célébration"
# (micro_scan_confirm_goal ci-dessus), qui s'est révélée inefficace sur
# caméra grand angle : un joueur qui célèbre occupe trop peu de pixels
# pour être vu de façon fiable (constaté empiriquement le 12/09/2026 —
# Gemini classait "attente" la frame même à l'instant exact où une
# célébration bras-levés était confirmée par capture d'écran).
#
# Approche retenue : au lieu de chercher un signal fin invisible en plan
# large, RE-VÉRIFIER le signal macroscopique qui fonctionne déjà
# (la formation RESTART_KO elle-même) sur PLUSIEURS frames autour du
# candidat, avec un vote majoritaire — le même principe que le système
# KO1/KO2 existant (qui vote sur plusieurs passes plutôt que de se fier
# à une seule classification), appliqué ici à un candidat de but en
# milieu de match.

PROMPT_MICRO_SCAN_RESTART_KO = """Analyse football match — {n} images extraites autour de t={t_candidat} (candidat de coup d'envoi/reprise au centre detecte par un premier passage), numerotees frame_0 a frame_{n_moins_1} dans l'ordre chronologique.

Pour CHAQUE frame, verifie si elle montre un VRAI coup d'envoi au centre du terrain, selon ces criteres STRICTS (TOUS obligatoires) :

1. BALLON IMMOBILE, exactement au point/rond central (pas en mouvement, pas juste "quelque part pres du centre").
2. JOUEURS NON MELANGES PAR EQUIPE, avec tolerance pour les traineurs — sur un coup franc (meme loin du but), les joueurs des deux equipes sont generalement MELANGES ensemble pres du ballon ; sur un coup d'envoi, LA MAJORITE des joueurs d'une equipe sont d'UN cote et LA MAJORITE des joueurs de l'autre equipe sont de L'AUTRE cote — quelques joueurs encore en transition sont NORMAUX et NE DOIVENT PAS faire rejeter un vrai coup d'envoi. Seul un melange GENERALISE doit faire rejeter.
3. Joueurs RELATIVEMENT STATIQUES ou en train de se placer calmement (pas en pleine course/action de jeu).

Si le MOINDRE de ces 3 criteres n'est pas clairement rempli sur une frame donnee, cette frame n'est PAS un coup d'envoi.

Reponds pour CHAQUE frame par "oui" ou "non" (pas de phrase), puis conclus.

Reponds UNIQUEMENT en JSON valide, sans texte avant/apres, sans balises markdown :
{{"votes_par_frame": {{"0": "oui", "1": "non", "...": "..."}}, "n_oui": <nombre entier de "oui">, "n_total": {n}}}
Le dictionnaire "votes_par_frame" DOIT contenir exactement {n} entrees (cles "0" a "{n_moins_1}")."""


def micro_scan_confirm_restart_ko(video_path, t_candidat, marge=8.0,
                                    pas_echantillon=2.0, seuil_vote=0.5,
                                    model_name="gemini-3.5-flash",
                                    jpeg_quality=75):
    """
    Re-vérifie un candidat RESTART_KO (issu du macro-scan) en votant sur
    PLUSIEURS frames autour de t_candidat, plutôt que de se fier à une
    seule classification isolée — corrige le faux positif confirmé le
    12/09/2026 sur Andrimont (t=895s), où une seule image ambiguë avait
    suffi à déclencher RESTART_KO à tort.

    Contrairement à micro_scan_confirm_goal (qui cherche un signal fin
    dans une fenêtre AVANT le candidat), cette fonction re-vérifie le
    signal macroscopique du candidat LUI-MÊME, sur une petite fenêtre
    centrée sur t_candidat.

    Args:
        video_path      : chemin de la vidéo
        t_candidat      : timestamp absolu du candidat RESTART_KO
                          (issu du macro-scan), secondes
        marge           : demi-largeur de la fenêtre autour de
                          t_candidat (ex. 8.0 → fenêtre de 16s)
        pas_echantillon : espacement cible entre deux frames (secondes)
        seuil_vote      : proportion minimale de "oui" pour confirmer
                          (0.5 = majorité simple)
        model_name      : modèle Gemini à utiliser
        jpeg_quality    : qualité JPEG d'encodage

    Returns:
        dict {"confirme": bool, "n_oui": int, "n_total": int,
              "votes_par_frame": dict, "n_appels_gemini": int}
        ou None si l'extraction vidéo échoue
    """
    import cv2
    from google import genai
    ...
    window_start = max(0.0, t_candidat - marge)
    window_end = t_candidat + marge
    duree_fenetre = window_end - window_start

    n_frames = max(2, int(duree_fenetre / pas_echantillon) + 1)
    offsets = [i * duree_fenetre / (n_frames - 1) for i in range(n_frames)]
    ...
    if not parts:
        print(f"  [MICRO_SCAN_KO] ⚠️ aucune frame extraite autour de t={t_candidat:.0f}s")
        return None

    prompt = PROMPT_MICRO_SCAN_RESTART_KO.format(
        n=len(parts), n_moins_1=len(parts) - 1, t_candidat=f"{t_candidat:.0f}s",
    )

    print(f"  [MICRO_SCAN_KO] Re-vote sur candidat t={t_candidat:.0f}s "
          f"({len(parts)} frame(s), 1 appel Gemini)...")
    ...
    try:
        ...
        print(f"  [MICRO_SCAN_KO] ⚠️ erreur Gemini : {e}")
        return {"confirme": False, "n_oui": 0, "n_total": len(parts),
                "votes_par_frame": {}, "n_appels_gemini": 1}

    if not data:
        print(f"  [MICRO_SCAN_KO] ⚠️ réponse invalide : {response.text[:150]!r}")
        return {"confirme": False, "n_oui": 0, "n_total": len(parts),
                "votes_par_frame": {}, "n_appels_gemini": 1}

    votes = data.get("votes_par_frame", {})
    n_oui = int(data.get("n_oui", sum(1 for v in votes.values() if str(v).lower() == "oui")))
    n_total = len(parts)
    confirme = (n_oui / n_total) >= seuil_vote if n_total > 0 else False

    print(f"  [MICRO_SCAN_KO] Votes par frame :")
    for i, off in enumerate(offsets_valides):
        _t_frame = window_start + off
        _vote = votes.get(str(i), "?")
        print(f"    frame_{i} (t≈{_t_frame:.0f}s) : {_vote}")

    _verdict = "CONFIRMÉ" if confirme else "REJETÉ (faux positif)"
    print(f"  [MICRO_SCAN_KO] → {_verdict} ({n_oui}/{n_total} votes 'oui')")

    return {
        "confirme": confirme,
        "n_oui": n_oui,
        "n_total": n_total,
        "votes_par_frame": votes,
        "n_appels_gemini": 1,
    }
