...
def infer_shots_from_goals(events):
    ...
    Priorité :
      ...
      2. N'importe quel event dans les 3s avant
      3. Fallback direct depuis la position du but
    """
    ...
    for g in all_sorted:
        ...
        # ── Priorité 2 : n'importe quel event dans les 3s ──
        if shot is None:
            near = [
                e for e in all_sorted
        ...
        # ── Priorité 3 : fallback direct depuis le but ──
        if shot is None:
            shot = {
                "type":     "shot",
                "time":     max(0.0, t - 1.0),
...
def filter_goals(
    ...
    4. Position trop loin des cages → rejeté
       Exception : Gemini conf >= 0.9
    """
    all_sorted     = sorted(events, key=lambda x: x.get("time", 0))
