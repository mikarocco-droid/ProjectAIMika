...
def detect_teams_preview(video_path, output_dir="outputs/preview",
    ...
    for pid, (j_list, s_list) in bgr_colors_by_pid.items():
        if j_list:
            bgr_final[pid] = (
                np.median(np.array(j_list, dtype=np.float32), axis=0),
                np.median(np.array(s_list, dtype=np.float32), axis=0) if s_list else None
            )
    bgr_colors_by_pid = bgr_final
    ...
    for ci in [0, 1]:
        cluster_pids = [stable_pids[i] for i in range(len(stable_pids))
                        if i < len(labels) and labels[i] == ci]
        j_list = [bgr_colors_by_pid[p][0] for p in cluster_pids
                  if p in bgr_colors_by_pid and bgr_colors_by_pid[p][0] is not None]
        s_list = [bgr_colors_by_pid[p][1] for p in cluster_pids
                  if p in bgr_colors_by_pid and bgr_colors_by_pid[p][1] is not None]

        j_color = tuple(int(x) for x in np.median(np.array(j_list, dtype=np.float32), axis=0)) if j_list else None
        s_color = tuple(int(x) for x in np.median(np.array(s_list, dtype=np.float32), axis=0)) if s_list else None

        if ci == 0: c0_j, c0_s = j_color, s_color
        else:        c1_j, c1_s = j_color, s_color
