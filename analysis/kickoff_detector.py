...
def find_kickoff_offset(events, video_duration_s, frames_data=None, fps=25,
    ...
    # On cherche le DERNIER groupe long satisfaisant :
    #   - sep >= _KP_SEP_MIN  (équipes bien séparées)
    #   - n   >= _KP_N_MIN    (assez de joueurs trackés)
    #
    # CONDITION PRÉALABLE : la vidéo doit commencer avant le match.
    # Si _p1_early_high=True (vidéo commence en jeu), le probe est inutile :
    #   - le kickoff n'existe pas dans la vidéo (offset=0 correct)
    #   - les groupes sep/n trouvés seraient de simples phases de match normale
    #   - andrimont_2 a démontré ce dérapage : t=5:53 retourné alors que KO=0s
    #
    ...
    if _p1_early_high:
        print(f"  [KICKOFF PLAYERS] probe désactivé — vidéo commence en jeu "
              f"(_p1_early_high=True) → offset=0 correct, signal joueurs non pertinent")
    else:
        ...
        if _kp_long_groups:
            ...
            # Candidat = début du PREMIER groupe après min_t
            # Raisonnement validé sur andrimont_full (307s réel) :
            #   - groupe 0 = vrai coup d'envoi (premier positionnement des équipes)
            #   - groupe N = reprises après buts, mi-temps → faux positifs tardifs
            # Ancien comportement [-1] retournait 855s (reprise) au lieu de 287s (KO)
            ...
            if _n_groups > 1:
                _last_t = f"{int(_kp_long_groups[-1]['t_start']//60)}:{int(_kp_long_groups[-1]['t_start']%60):02d}"
                print(f"  [KICKOFF PLAYERS] → {_n_groups} groupe(s) : candidat kickoff t={_t_fmt} "
                      f"(premier groupe) | dernier groupe ignoré={_last_t}")
            else:
